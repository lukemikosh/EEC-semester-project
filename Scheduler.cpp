...
void shutoffUnused(){
    ...
    for(unsigned CPUType = 0; CPUType < 4; CPUType++){
        for(unsigned gpu = 0; gpu < 2; gpu++){
            vector<MachineId_t> machines = CPUtoMachine[(CPUType_t) CPUType][(bool) gpu];
            for(unsigned i = machines.size() / 4; i < machines.size(); i++){
                
                if(machineToVM[i].size() == 0 && Machine_GetInfo(i).s_state == S0 && !waking[i] && !offline[i]){
                    offline[i] = true;
...
void chainMigrate(){
    ...
    for(unsigned CPUType = 0; CPUType < 4; CPUType++){
        ...
        for(unsigned gpu = 0; gpu < 2; gpu++){
            ...
            sort(machinesCopy.begin(), machinesCopy.end(), [](MachineId_t a, MachineId_t b)
            {return machineDataVect[a].task_count * Machine_GetInfo(b).num_cpus <  machineDataVect[b].task_count * Machine_GetInfo(a).num_cpus; });


            unsigned index = 0;
            for(unsigned i = 0; i < machinesCopy.size(); i++){
                if(machineDataVect[machinesCopy[i]].task_count > 0 && machineDataVect[i].task_count < machineDataVect[i].max_tasks / 3){
                    index = i;
                    break;
                }
            }
            ...
            while(index < machinesCopy.size()){
                ...
                for(unsigned dest_index = machinesCopy.size() - 1; dest_index > index; dest_index--){
                    ...
                    unsigned dest_vm_count = machineToVM[destination_id].size();
                
                    if(destination_machine_data.task_count == destination_machine_data.max_tasks || dest_vm_count >= VMS_PER_MACHINE || offline[destination_id]){
                        continue;
                    }
                    ...
                    for(unsigned source_vm_index = 0; source_vm_index < source_vms.size(); source_vm_index++){
                        ...
                        migrationMap[source_vm_id] = destination_id;
                ...
                if(machineToVM[source_machine_id].size() == migrationMap.size()){

                    for(auto it = migrationMap.begin(); it != migrationMap.end(); it++){
                        VMId_t source_vm_id = it->first;
                        MachineId_t destination_id = it->second;
                        struct VMData source_vm_data = VMDataMap[source_vm_id];
                        struct MachineData destination_machine_data = machineDataVect[destination_id];
